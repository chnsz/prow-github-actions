 ...
 * @param issueNum - the issue associated with this runtime
 */
export const getCurrentLabels = async (context: Context, issueNum: number): Promise<string[]> => {
    const octokit = newOctokit()
    try {
        /* eslint-disable @typescript-eslint/naming-convention */
 ...
 * @param context - the github actions event context
 * @param issueNum - the issue associated with this runtime
 * @param label - the label to remove from the issue
 */
export const cancelLabel = async (
    ...
    try {
        currentLabels = await getCurrentLabels(context, issueNum)
    ...
    if (currentLabels.includes(label)) {
        try {
            await removeLabels(context, issueNum, [label])
        } catch (e) {
